...
\usetikzlibrary{patterns}											%mit Mustern
\usetikzlibrary{decorations.pathreplacing,decorations.pathmorphing} %und so
\usepackage[flushmargin, hang, bottom]{footmisc}						%Fußnotenstyle
\usepackage{amsfonts}													%Schriften f\"{u}r den Mathematikmodus
...
\usepackage{babel} %Silbentrennung, Benennung des Inhaltsverz. etc.
\usepackage[utf8]{inputenc}  		%Liest die Umlaute und das scharfe s
\usepackage{colortbl}		    	%Farben in Tabellen
\usepackage{umoline}			    %Neue Überlinie ohne Mathemodus
\usepackage{stmaryrd}				%Für die eckigen Klammern (Ext. und Int.).
\usepackage{tabularx}    			%Tabelle mit fester Breite und variablen Spalten.
\usepackage{multirow}				%Mehrere Zeilen umfassende Spalten
\usepackage[linguistics]{forest}					%Der Baumzeichner (Latex for Linguists)
\usepackage{qtree}					%Der Baumzeichner (Latex for Linguists)
\usepackage[normalem]{ulem}			%Bessere Unter- und Überstreichung
\usepackage[hypertexnames=false]{hyperref}		%Navigierbare pdf
...
\begin{document}
	...
	\begin{exe}
		\ex \begin{itemizeexamplecommand}
			...
			{\scriptsize\item[] Arguments:\begin{itemizeexamplecommand}
				...
				\end{itemizeexamplecommand}
				\item[] Description: Creatin double square bracketing for an expression without a situation argument.
				\item[] Bold Option: Yes
	...
	\begin{exe}
		\ex \begin{itemizeexamplecommand}
			...
			{\scriptsize\item[] Arguments:\begin{itemizeexamplecommand}
					...
					\item[] \texttt{expression}: the expression to interpret.
				\end{itemizeexamplecommand}
				\item[] Description: Creatin double square bracketing for an expression with a situation argument.
				\item[] Bold Option: Yes
	...
	\begin{exe}
		\ex \begin{itemizeexamplecommand}
			...
			{\scriptsize\item[] Arguments:\begin{itemizeexamplecommand}
				...
				\end{itemizeexamplecommand}
				\item[] Description: Creates a replacement.
				\item[] Bold Option: Yes
	...
	\end{exe}
	\subsection{semtree}
	The \textit{semantic-tree} environment is an optional environment to add tree structures to any derivation of a direct or indirect semantic expressions. So far no custom tree generating
	commands are implemented, all tree packages should work 
	\begin{exe}
		\ex \begin{itemizeexamplecommand}
			\item[] \begin{lstlisting}[style=B]
			\begin{semtree}[label]
	...
	\begin{exe}
		\ex \begin{itemizeexamplecommand}
			\item[]
			\begin{lstlisting}[style=B]O{} O{x} O{y} O{\sstar} O{\@typee} O{\@typee} O{}
	...
	\begin{exe}
		\ex \begin{itemizeexamplecommand}
			\item[]
			\begin{lstlisting}[style=B]O{} O{p} O{x} O{j} O{i} O{\@typest} O{\@typee} O{\@types} O{}
	...
	\end{exe}
	\subsection{semderi}
	The \textit{semantic-derivation} environment is the heart of the \FraGoLi\ package and is used for fast and easy derivation of a direct or indirect semantic expressions. 
	\begin{exe}
		\ex \begin{itemizeexamplecommand}
			\item[] \begin{lstlisting}[style=B]
			\begin{semderi}[label]
	...
	\end{exe}
	\subsubsection{Constants}
	This section will list all pre-defined commands acting as constants for common derivational steps in the \textit{semderi} environment. They should be used as sidenote. If the language gets changed via the package options, those constants will be translated.
	\begin{center}
		\begin{minipage}[h][3.8cm][t]{15em}
			\begin{lstlisting}[style=B]
			\fa
	...
	\begin{exe}
		...
		\end{itemizeexamplecommand}
	\end{exe}
	\begin{exe}\label{command:resourceCounter}
		\ex \begin{itemizeexamplecommand}
			\item[]
			\begin{lstlisting}[style=B]
			\rr{resourceCounter}
